...
import { ASSISTANT_ID, DEFAULT_VOICE, VOICE_SETTINGS } from '@/constants';
...
export function useVapi(book: IBook) {
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            if (sessionIdRef.current) {
                endVoiceSession(sessionIdRef.current, 0).catch((endErr) => console.log("Could not rollback voice session after start process did not suceed: ", endErr));
                sessionIdRef.current = null;
            }
            setStatus('idle');
